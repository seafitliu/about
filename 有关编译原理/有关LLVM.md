...
##C/C++核心语法
- 每个C程序源码文件，连同被包含的头文件，构成一个翻译单元（translation unit）
- C语言没有换行和缩进的规则（与python不同），你可以使用空格，制表符，空白行将源代码隔开
- 预处理指令，一定要出现在独立一行中，#之前只能有空格或制表符
- C语言中有两种注释：/**/块式注释和//单行式注释(C99);字面值""之中包含，不作为注释；注释会被预处理器替换成空格
- C语言中保留了37个关键字，这些不能作为标识符，如下：
- 保留的标示符
	- 下划线开头，紧接第二个下划线或大写字母，一律被保留，例如\__function__, _M
	- 下划线开头，xxxxx
- 标识符的范围，有4种：
- 整型
- 浮点型
